...
class DeletePageRule : AccessRule<Page, PageExecutableAction, WorkspaceScopedAccessor>() {
    override fun isApplicableForRequest(request: PageExecutableAction): Boolean {
        return PageExecutableAction.DELETE_PAGE == request
    }
    ...
    override fun definition(): AccessRuleContext<Page, PageExecutableAction, WorkspaceScopedAccessor>.() -> Unit = {
        ...
        exitWithFailureIfTrue("Page has been deleted already") {
        ...
        exitWithSuccessIfTrue("Member has permission to delete this page") {
            ...
            effectivePermission?.hasWorkspacePermissionOrHigher(WorkspaceMember.Permissions.WorkspacePermission.CAN_ADMINISTRATE) == true
                    || effectivePermission?.hasPagePermissionOrHigher(WorkspaceMember.Permissions.PagePermission.CAN_VIEW_CREATE_EDIT_DELETE) == true
                    || (input?.accessor?.member?.userUUID == input?.entity?.creatorUUID
