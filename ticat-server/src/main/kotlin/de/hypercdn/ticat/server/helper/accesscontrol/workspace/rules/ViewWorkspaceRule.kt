...
class ViewWorkspaceRule : AccessRule<Workspace, WorkspaceExecutableAction, WorkspaceAccessor>() {
    ...
    override fun definition(): AccessRuleContext<Workspace, WorkspaceExecutableAction, WorkspaceAccessor>.() -> Unit = {
        ...
        exitWithFailureIfTrue("Workspace has been deleted") {
        ...
        exitWithSuccessIfTrue("Member has permission to view this workspace") {
            val effectivePermission = input?.accessor?.member?.effectivePermission()
            effectivePermission?.hasWorkspacePermissionOrHigher(WorkspaceMember.Permissions.WorkspacePermission.CAN_VIEW) == true
        }
    }
