...
class DeleteWorkspaceRule : AccessRule<Workspace, WorkspaceExecutableAction, WorkspaceAccessor>() {
    override fun isApplicableForRequest(request: WorkspaceExecutableAction): Boolean {
        return WorkspaceExecutableAction.DELETE_WORKSPACE == request
    }
    ...
    override fun definition(): AccessRuleContext<Workspace, WorkspaceExecutableAction, WorkspaceAccessor>.() -> Unit = {
        ...
        exitWithFailureIfTrue("Workspace has been deleted already") {
            input?.entity?.isDeleted() == false
        }
        ...
        exitWithSuccessIfTrue("Member has permission to administrate this workspace") {
